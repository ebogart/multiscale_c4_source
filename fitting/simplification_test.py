...
full_model.set_bounds({'tx_A': (0., 1000.),
                       ...
                       'extra_var0': (-5., 5.),
                       'extra_var1': (-5., 5.)})
# Note extra_var1 is just there to ensure the number of variables
# is greater than the number of equality constraints.

# What should happen when this network is simplified:
# Reactions R1 and R3 are blocked because only R3 produces/consumes 
# metabolite C; their values should be fixed to zero. Constraints
# _conservation_B and equal_squares_0 involve only R1 and R3, 
# so they should be dropped from the problem. R3 participates
# in no other constraints and should be dropped from the problem 
# in turn; R1 participates in equal_squares_1 and should
# be made a parameter with value 0. 
#
# With R1 and R3 eliminated, the constraints conserving F and G
# become redundant, and one or the other should be eliminated.
#
# Finally, the variable bounds on tx_A and sink are mutually redundant, 
# and one or the other should be relaxed; R2 is redundant given the remaining
# tx_A/sink constraint and it should be relaxed too.
