...
# Fit.
t1 = time.ctime()
replica._fit(step_limit=12,rs_max_iter=250,rs_max_attempts=10)
t2 = time.ctime()

# Postprocessing step: Detect cases where some fluxes are stuck at 
# zero and dislodge them
preliminary_result = replica.fit_result.copy()
preliminary_soln = replica.fitting_model.soln.copy()
false_zeros = [k for k,v in replica.fit_result.iteritems() if
               np.max(np.abs(v)) < 1e-4 and k in replica.data and
               np.mean(replica.data[k]) > 0.] 
error_bounds = {}
error_bound_factor = 0.99
for r in false_zeros:
    for i in xrange(15):
        error_variable = 'image%d_gradient_rna_error_%s' % (i,r)
        datum = replica.data[r][i]
        error_bounds[error_variable] = (None, error_bound_factor*datum)
replica.fitting_model.set_bounds(error_bounds)
t3 = time.ctime()
replica.fit_log.append('Starting postprocessing step %s' % t3)
replica.fitting_model.repeated_solve(
    replica.fitting_model.x, max_iter=250,
    max_attempts=10
)
t4 = time.ctime()
replica.fit_log.append('Ending postprocessing step %s' % t4)
replica.fit_log.append('Final value %.3g' % 
                       replica.fitting_model.obj_value)

replica._fit_postprocess()
...
result = {
    ...
    'preliminary_soln': preliminary_soln,
...
# Label the result with year, day of year and process id
pid = getpid()
year = time.localtime()[0]
yday = time.localtime()[-2]
filename = 'gradient_fit.pickle'

with open(filename,'w') as f:
    pickle.dump(result,f)

# Output CSV for inclusion as paper supplementary file
with open('gradient_fit.csv','w') as f:
    for variable in _base_model.variables:
        # Variables which are fixed to zero by the constraints
        # have been dropped from the fitting model.
        values = replica.fit_result.get(variable, np.zeros(15))
        line = variable + '\t' + '\t'.join(['%.18e' % v for 
                                            v in values]) + '\n'
        f.write(line)
